...
supabase.storage
  ...
  .then(({ data, error }) => {
    ...
    if (!bucketExists) {
      console.error(
        `❌ [STORAGE] Bucket "${STORAGE_BUCKET}" не найден`
      );
    }
  })
  .catch((error) => {
...
function getTeachingSession(
  userId
) {

  return teachingSessions.get(
    userId
  );
}

function setTeachingSession(
  userId,
  session
) {

  teachingSessions.set(
    userId,
    session
  );
}

function clearTeachingSession(
  userId
) {

  teachingSessions.delete(
    userId
  );
}
...
const identifySessions =
  new Map();

function getIdentifySession(
  userId
) {

  return identifySessions.get(
    userId
  );
}

function setIdentifySession(
  userId,
  session
) {

  identifySessions.set(
    userId,
    session
  );
}

function clearIdentifySession(
  userId
) {

  identifySessions.delete(
    userId
  );
}
...
async function getCharacterLore(
  ...
  console.log(
    `🔎 [LORE] Поисковый текст: "${loreSearchText}"`
  );
...
function buildCharacterLoreContext(
  rows
) {

  if (!rows.length) {
    return (
      "(релевантный лор персонажей не найден)"
    );
  }
...
async function generateImageEmbedding(
  ...
  if (
    ...
    throw new Error(
      "Пустое изображение для embedding."
    );
  }

  console.log(
    "🧬 [EMBEDDING] Генерируем embedding..."
  );
  ...
  console.log(
    `✅ [EMBEDDING] Получен вектор ${embedding.length}D`
  );
...
async function getPhotoBufferFromTelegram(
  telegram,
  fileId
) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Ошибка скачивания фото: HTTP ${response.status}`
    );
  }
...
async function matchCharacterPhotos(
  embedding,
  matchCount = 10
) {
...
function groupSimilarityCandidates(
  rows
) {
...
async function generateAIResponse(
  ...
  for (
    ...
    if (
      ...
      console.log(
        `⏸️ [GEMINI] ${model.name} пропущена — cooldown`
      );
    ...
    try {
      ...
      if (
        ...
        console.log(
          `⏳ [GEMINI] Временная ошибка у ${model.name}`
        );
      ...
      console.log(
        `➡️ [GEMINI] Ошибка постоянная, следующая модель`
      );
    }
  }
...
async function sendRadarMessage(
  ctx
) {
  ...
  try {
    ...
    console.log(
      `📡 [RADAR] msg=${ctx.message.message_id} → owner`
    );
  ...
  if (
    ...
    try {
      ...
      if (
        ...
        await ctx.reply(
          "✅ Все фотографии уже имеют embeddings."
        );
      ...
      for (
        ...
        try {
          ...
          const buffer =
            await getPhotoBufferFromTelegram(
              ctx.telegram,
  ...
  if (
    ...
    if (teaching) {
      ...
      await ctx.reply(
        ...
        `❌ Обучение персонажа ` +
        `${teaching.characterName} отменено.`
      ...
      for (
        ...
        try {
          ...
          console.error(
            ...
            `⚠️ [TEACH] Embedding фото #` +
            `${photoNumber} не создан:`,
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          "🔍 [IDENTIFY] Фото получено"
        );

        await ctx.reply(
          "🧬 Сравниваю фото с базой..."
        );
        ...
        const matches =
          await matchCharacterPhotos(
            embedding,
            10
          );

        const candidates =
          groupSimilarityCandidates(
            matches
          );
        ...
        console.log(
          ...
          `🔍 [IDENTIFY] ` +
          `Кандидатов: ${topCandidates.length}`
...
async function sendOwnerMessageToGroup(
  ctx
) {
...
async function handleAI(
  ...
  try {
    ...
    console.log(
      "📚 [LORE] Ищем персонажей в базе..."
    );

    const loreRows =
      await getCharacterLore(
        text
      );

    const characterLoreContext =
      buildCharacterLoreContext(
        loreRows
      );
